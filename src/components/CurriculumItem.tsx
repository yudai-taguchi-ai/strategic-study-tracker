...
import { FileText, Video, Link2 as LinkIcon, LayoutGrid, ChevronRight, Clock, ArrowUpRight } from 'lucide-react'
...
export function CurriculumItem({ material }: { material: Material }) {
    ...
    async function handleDetach(e: React.MouseEvent) {
        e.preventDefault()
        e.stopPropagation()
        if (!confirm('この教材を講座の外に移動しますか？')) return
        try {
            await detachMaterialFromCourse(material.id)
            router.refresh()
        } catch (err) {
            alert('エラーが発生しました')
        }
    ...
    return (
        <div className="relative group/container">
            ...
            >
                ...
                <div className="flex-1 min-w-0 pr-8"> {/* Padding for the button */}
            ...
            <button
                ...
                className="absolute top-2 right-2 p-1.5 bg-black/50 hover:bg-orange-500 text-white rounded-lg opacity-0 group-hover/container:opacity-100 transition-all z-10 border border-white/10"
                title="講座の外（ダッシュボード）に移動"
            >
